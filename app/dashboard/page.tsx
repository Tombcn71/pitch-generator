  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        setError(
          "Failed to generate pitch. Please check your API key and try again."
        );
      }
    } finally {
    ...
    const formattedLines = lines.map((line, index) => {
      ...
      if (line.startsWith("# ")) {
        return (
          <h1
            key={index}
            className="text-2xl font-bold mt-6 mb-4 text-primary border-b pb-2">
            {line.substring(2)}
          </h1>
        );
        ...
        return (
          <h2
            key={index}
            className="text-xl font-semibold mt-5 mb-3 text-primary/90">
            {line.substring(3)}
          </h2>
        );
      } else if (line.startsWith("### ")) {
        return (
          <h3
            key={index}
            className="text-lg font-medium mt-4 mb-2 text-primary/80">
            {line.substring(4)}
          </h3>
        );
      } else if (line.trim() === "") {
      ...
      <div className="space-y-1 pitch-content p-8 bg-card border rounded-md shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-muted-foreground">
            Based on David Beckett's Pitch Canvas
          </div>
        </div>
          ...
          <TabsContent value="form">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Fill in the details below to generate your 3-minute pitch
                </CardDescription>
              </CardHeader>
              <form onSubmit={handleSubmit}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          handleInputChange("traction", e.target.value)
                        }
                        required
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        value={formData.ask}
                        onChange={(e) =>
                          handleInputChange("ask", e.target.value)
                        }
                        required
                      />
                    </div>
                ...
                <CardFooter>
                  <Button type="submit" disabled={isGenerating}>
                    {isGenerating ? "Generating..." : t("pitch.generate")}
                  </Button>
                </CardFooter>
              </form>
            </Card>
          ...
          <TabsContent value="result">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Here&apos;s your AI-generated 3-minute pitch based on David
                  Beckett&apos;s method
                </CardDescription>
              </CardHeader>
              <CardContent>
